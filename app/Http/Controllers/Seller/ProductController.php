...
class ProductController extends Controller
{
    public function add_new()
    {
        ...
        $products = Product::where(['added_by' => 'seller', 'user_id' => \auth('seller')->id()])->latest()->paginate(12);
        ...
        if ($request->ajax()) {
            ...
            if ($request->file('images')) {
                foreach ($request->file('images') as $img) {
                    ...
                    $product_images[] = $request->image->move(public_path('product'), $imageName);
                }
            ...
            $product->thumbnail = $request->image->move(public_path('product'), $imageName);
    ...
    function list(Request $request)
    {
        ...
        $products_seller = Product::where(['added_by' => 'seller', 'child_seller_id' => \auth('seller')->id()]);
    ...
    public function search(Request $request)
    {
        ...
        $products = Product::where('user_id',auth('seller')->id())
    ...
    public function seller_product_update_price(Request $request, $id) {
    ...
    public function seller_product_update_price_file(Request $request){

        return view('seller-views.product.update_price');

    }


    public function seller_product_update_price_file_update(Request $request) {
          ...
          $get_file = $request->file->move(public_path('product/files'), $check);
            ...
            $products = (new FastExcel)->import($get_file, function ($line) {
                ...
                return Product::where('sku', '=', $line['sku'])->where('user_id', '=', auth('seller')->id())->update([

                    'name'          => $line['name'],
                    'slug'          => Str::slug($line['name']),
                    'unit_price'    => $line['price'],
                    'status'        => $line['available'],
                    'current_stock' => $line['stock'],
                    'unit'          => $line['unit'],
                    'unit_numbers'  => $line['unit numbers'],
                  
                ]);
            });
 

                //    $products = Product::insert($collections);
    ...
    public function seller_product_offers() {

        $products = Product::where(['added_by' => 'seller', 'child_seller_id' => \auth('seller')->id()])->latest()->paginate(10);

        return view('seller-views.product.offers', compact('products'));

    }



    public function seller_add_product_discount(Request $request) {
        ...
        $product = Product::where('id', '=', $myId)->update([

            'discount'          => $discount,
            'discount_percent'  => $request->discount,
            'discount_start'    => $request->discount_start_date,
            'discount_end'      => $request->discount_end_date
    ...
    public function seller_product_discount_delete(Request $request) {
        ...
        Toastr::success('تم حذف الخصم بنجاح');
        return redirect()->back();
    ...
    public function seller_product_add_offer(Request $request) {


        $myId = $request->myId;

        $discount = $request->unit_price - $request->price_after_discount;

        $product = Product::where('id', '=', $myId)->update([

            'discount'          => $discount,
            'offer_price'       => $request->price_after_discount,
            'offer_start'       => $request->offer_start_date,
            'offer_end'         => $request->offer_end_date

        ]);

       

        Toastr::success('تم اضافة العرض بنجاح');
        return redirect()->back();
    ...
    public function seller_product_offer_delete(Request $request) {
