...
class ProductManager
{
    ...
    {
        $paginator = Product::with(['rating'])->whereIn('status', ['1'])->latest()->paginate($limit, ['*'], 'page', $offset);
        /*$paginator->count();*/
        return [
    ...
    public static function get_best_selling_products($limit = 10, $offset = 1)
    {
        ...
        if(count($orders) > 0) {
            ...
            foreach($orders as $val) {
                ...
                $products = Product::where('id', '=', $val->product_id)->first();
    ...
    public static function get_discounted_product($limit = 10, $offset = 1)
    {
        ...
        $get_pro = DB::table('products')->where('discount', '!=', 0)->get();

        return [
            'total_size' => count($get_pro),
            'limit' => (integer)$limit,
            'offset' => (integer)$offset,
            'products' => $get_pro
        ];
