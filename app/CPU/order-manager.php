...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        $timeToDeliverysession = session()->has('timeToDelivery') ? session('timeToDelivery') : null;
        $daysToDeliverysession = session()->has('daysToDelivery') ? session('daysToDelivery') : null;
        
        if(isset($data['timeToDelivery']))
        {
            $timeToDelivery = $data['timeToDelivery'];
        } elseif (isset($timeToDeliverysession)){
            $timeToDelivery = $timeToDeliverysession;
        } else {
            $timeToDelivery = 0;
        }
        if(isset($data['daysToDelivery']))
        {
            $daysToDelivery = $data['daysToDelivery'];
        } elseif (isset($daysToDeliverysession)){
            $daysToDelivery = $daysToDeliverysession;
        } else{
            $daysToDelivery = 0;
        }
