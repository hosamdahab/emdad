...
if(auth('customer')->check()) {
    ...
    $cart = \App\Model\Cart::where('customer_id', '=', $user->id)->get();
    $cart_count = \App\Model\Cart::where('customer_id', '=', $user->id)->count('id');
    $cart_subTotal = \App\Model\Cart::where('customer_id', '=', $user->id)->sum('total');
    $Wishlist = \App\Model\Wishlist::where('customer_id', '=', $user->id)->count('id');
...
<div class="navbar-tool dropdown {{Session::get('direction') === "rtl" ? 'mr-3' : 'ml-3'}}"
     style="background:#f8f8f8;border-radius:50%;padding:3px;margin-{{Session::get('direction') === "rtl" ? 'left' : 'right'}}: 6px">
        ...
        @if(auth('customer')->check())
            ...
            @if(count($cart) > 0)
                <div>
                    ...
                    $Pro_id = \App\Model\Product::where('id', '=', $cartItem->product_id)->first()
                   ?>
                        <div class="widget-cart-item pb-2">
                            <form action="{{route('remove.cart.item', ['id' => $cartItem->id])}}" method="post" id="remove_cart_item">
                                @csrf
                                <input type="hidden" name="myId" value="{{$cartItem->id}}">
                            ...
                            <div class="media">
                                ...
                                <div class="media-body">
                                    ...
                                    <h6 class="widget-product-title">
                                        <a href="{{route('product.view', ['id' => $Pro_id->id])}}">{{$cartItem->name}}</a></h6>
                                   
                                    <div class="widget-product-meta d-flex justify-content-end">
                                        <span
