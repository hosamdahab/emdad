...
@section('content')
    ...
    <div class="d-flex justify-content-center align-items-center mb-3" style="min-height: 70px;background:{{$web_config['primary_color']}}10;width:100%;">
            ...
            <div class="row text-capitalize">
                <span style="font-weight: 600;font-size: 18px;">@isset($Category->name) {{$Category->name}} @endisset</span>
            </div>
   ...
   <div class="row">
    <div class="sub-cats col-12" style="display:flex;justify-content:flex-end; margin-right:20px;padding:50px">
       ...
       @isset($sub_cats)
        ...
        <form action="{{route('sub.category.search')}}" method="post" id="sub_filters">
            ...
            <button type="submit" id="all" class="btn" style="1px solid #ecedee;#6c6f7f;font-weight:700;font-size:16px;background:white;border-radius:8px;padding:9px 24px;margin:0 15px">{{$val->name}}</button>
        </form>
        ...
        <a href="{{route('categories')}}" id="all" class="btn" style="background-color:#5044b8;color:#ffffff">الكل</a>
    </div>
   </div>
                        ...
                        <div class="col-md-6 m-2 m-md-0 d-flex  align-items-center ">
                            ...
                            <div class="" style="width: 100%">
                                <form id="filters-search-form" action="{{ route('product.filters.serach') }}" method="POST">
                                    ...
                                    <input type="hidden" name="category_ids" id="category_ids" value="{{$Category->id}}">
                                </form>
                            </div>
                        </div>
                    </div>
                {{-- </div> --}}
                   ...
                   <!-- Get Products -->
                    ...
                    <div class="row" id="pro_row">
                       ...
                       $shop_info = DB::table('shops')->where('seller_parent', '=', $pro->user_id)->first();
                       $user = App\CPU\Helpers::get_customer();

                       $Brand = DB::table('brands')->where('id','=', $pro->brand_id)->first();
                        $Category = DB::table('categories')->where('id', '=', $pro->category_ids)->first();
                       ...
                       <div class="card" style="width: 18rem;">
                            ...
                            <div class="card-body">
                                ...
                                <h5 class="card-title" style="padding:0;margin:0"><a href="{{route('product.view', ['id' => $pro->id])}}">{{$Brand->name}} {{$Category->name}} {{$pro->name}}</a></h5>
                                </section>
                                @else 
                                <section class="d-flex">
                                ...
                                <span><strong>{{$pro->unit_price. ' ريال'}} </strong> ({{($pro->unit_price / $pro->carton_unit).' ريال للحبة'}})</span>
                                </section>
                                <br>
...
@push('script')
 ...
 <script>
    ...
    $(document).ready(function(){
        ...
        $('#pro_filters').on('change',function(){
           
            var pro_filters = $(this).val();

            var category_ids = $('#category_ids').val();

            $.ajax({
                type:'POST',
                url: "{{ route('product.filters.serach') }}",
                data: {'pro_filters': pro_filters, 'category_ids': category_ids},
                success: (response) => {
                    ...
                    $.each(response.product,function(key,item){
                        ...
                        $('#pro_row').append('<div class="card" style="width: 18rem;">\
                            <a href="/product/" + item.id +"">\
                            <img src="{{asset('product/thumbnail/')}}/'+ item.thumbnail +'" draggable="false" class="card-img-top" alt="" style="width:100%;height:200px">\
                            </a>\
                            <div class="card-body">\
                                <section class="d-flex">\
                                <h5 class="card-title" style="padding:0;margin:0"><a href="/product/'+ item.id +'">"' + item.brand_id + item. '+ item.name +'</a></h5>\
                                </section>\
                                \
                                <section class="d-flex">\
                                ...
                                \
                                <section class="d-flex">\
                                <span>'+ item.carton_unit +'</span>\
                                <span> x </span>\
                                <span>'+ item.unit_numbers +' '+ item.unit +'</span>\
                                </section>\
                                \
                                <section class="d-flex" style="margin-top:20px">\
                                <span><strong>'+ item.unit_price + ' ريال' +'</strong>'+ ' ' +  ' = ' +  (item.unit_price / item.carton_unit + ' للحبة') + '</span>\
                                </section>\
                                <br>\
                                <a href="#" class="btn btn-primary">{{__('messages.add_cart')}}</a>\
                            </div>\
                        </div>\
                        ');
